...
* Control group: 10% fails to solve the the first puzzle, 40% solve only 1 puzzle, 40% solve 2 puzzles, 10% solve 3 puzzles.
* Treatment group: 10% fails to solve the first puzzle, 20% solve only 1 puzzle, 40% solve 2 puzzles, 20% solve 3 puzzles, 10% solve 4 puzzles.
* No correlation between any covariates on the outcome.

## High effect size (1.5), high dispersion
* Control group: 10% fails to solve the the first puzzle, 40% solve only 1 puzzle, 40% solve 2 puzzles, 10% solve 3 puzzles.
* Treatment group: 0% fails to solve the first puzzle, 10% solve only 1 puzzle, 20% solve 2 puzzles, 40% solve 3 puzzles, 20% solve 4 puzzles, 10% solve 5 puzzles.
* No correlation between any covariates on the outcome.
...
d[treat == 0,  outcome_1 := sample(c(0, 1, 2), size = .N, replace = TRUE, prob = c(.1, 0.8, 0.1))]

# Create treatment outcome
d[treat == 1,  outcome_1 := sample(c(0, 1, 2, 3), size = .N, replace = TRUE, prob = c(.1, 0.4, 0.4, 0.1))]

# Scenario 2
# Create control outcomes
d[treat == 0,  outcome_2 := sample(c(0, 1, 2, 3), size = .N, replace = TRUE, prob = c(.1, 0.4, 0.4, 0.1))]

# Create treatment outcome
d[treat == 1,  outcome_2 := sample(c(0, 1, 2, 3, 4), size = .N, replace = TRUE, prob = c(.1, 0.2, 0.4, 0.2, 0.1))]

# Scenario 3
# Create control outcomes
d[treat == 0,  outcome_3 := sample(c(0, 1, 2, 3), size = .N, replace = TRUE, prob = c(.1, 0.4, 0.4, 0.1))]

# Create treatment outcome
d[treat == 1,  outcome_3 := sample(c(0, 1, 2, 3, 4, 5), size = .N, replace = TRUE, prob = c(0, .1, 0.2, 0.3, 0.2, 0.1))]

```

# Statistical Power

```{r many ten person samples, echo=FALSE}
...
for(sample_size in sample_sizes){
  ...
  ## (which is an easy-of-use wrapper to an apply method)
    ...
    for(sim in 1:n_loops) { 
      ...
      regression_p_values_1[sim] <- summary(lm(outcome_1 ~ treat, data = combined_group))$coefficients[2,4] 
      
      regression_p_values_2[sim] <- summary(lm(outcome_2 ~ treat, data = combined_group))$coefficients[2,4]
      
      regression_p_values_3[sim] <- summary(lm(outcome_3 ~ treat, data = combined_group))$coefficients[2,4]
...
colors <- c("Senario 1" = "blue", "Senario 2" = "red", "Senario 3" = "green")
ggplot(df, aes(x = sample_sizes*2)) +
  geom_line(aes(y = power_sample_size_1, color = "Senario 1")) +
  geom_line(aes(y = power_sample_size_2, color = "Senario 2")) +
  geom_line(aes(y = power_sample_size_3, color = "Senario 3")) +
  labs(x = "Sample Size",
         y = "Power",
         color = "Legend") +
